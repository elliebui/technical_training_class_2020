...
def get_most_common_month(people):
    ...
    for person in people:
        if person.birthday_month in months:
            ...
            months.update({person.birthday_month: months[person.birthday_month]})

    max_value = None
    for month, number_of_birthday in months.items():
        if max_value is None:
            max_value = number_of_birthday
        elif number_of_birthday > max_value:
            max_value = number_of_birthday

    result_month = [month for month, number_of_birthday in months.items() if number_of_birthday == max_value]

    return result_month[0]
